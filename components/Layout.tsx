...
const Layout: React.FC<LayoutProps> = ({ children, user, onLogout, onLoginClick, activeTab, setActiveTab }) => {
  ...
  const adminNav = [
    ...
    { id: 'team_management', label: 'Ufficio Personale', icon: 'fa-user-tie' },
  ...
  let navItems = isGuest ? [{ id: 'dashboard', label: "Kristal", icon: 'fa-concierge-bell' }] : clientNav;
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#fcfcfc] overflow-x-hidden">
            ...
            <div className="space-y-4 border-t border-gray-50 pt-8">
              <div className="flex items-center space-x-3 px-2">
                ...
                <div className="overflow-hidden">
                  ...
                  <p className="text-[8px] text-amber-600 font-bold uppercase tracking-widest">{isAdmin ? 'Direzione' : 'Staff'}</p>
                </div>
              </div>
              <button onClick={onLogout} className="w-full flex items-center space-x-3 px-4 py-3 text-gray-400 hover:text-red-500 transition-all">
                <i className="fas fa-sign-out-alt text-xs"></i>
                <span className="text-[9px] font-bold uppercase tracking-[0.3em]">Congedarsi</span>
              </button>
            </div>
          ) : (
            <button onClick={onLoginClick} className="w-full py-5 bg-black text-white rounded-2xl text-[10px] font-bold uppercase tracking-widest shadow-xl hover:bg-amber-600 transition-all">Accedi</button>
          )}
        </div>
      </aside>

      <main className="flex-1 p-4 md:p-16 bg-[#fcfcfc] w-full pb-32 md:pb-16">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-gray-100 px-2 py-3 flex justify-around items-center z-[500] shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
        {navItems.map((item) => (
          <button key={item.id} onClick={() => setActiveTab(item.id)} className={`flex flex-col items-center space-y-1 transition-all flex-1 ${activeTab === item.id ? 'text-black scale-110' : 'text-gray-300'}`}>
