    ...
    <div className="w-full bg-white rounded-[4rem] shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
      <div className="p-10 md:p-14 overflow-y-auto scrollbar-hide">
        <div className="text-center mb-10">
          <h1 className="text-4xl font-luxury font-bold text-gray-900 tracking-tighter mb-2">SALONE</h1>
        ...
        <form onSubmit={handleAuth} className="space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {isRegistering && (
              <>
                <div className="space-y-1">
                  ...
                  <input id="reg-fullname" name="name" type="text" required value={fullName} onChange={(e) => setFullName(e.target.value)} className="w-full p-4 rounded-2xl bg-gray-50 border-none outline-none text-xs font-bold" />
                </div>
                <div className="space-y-1">
                  <label htmlFor="reg-phone" className="text-[8px] font-bold text-gray-400 uppercase ml-2">Telefono</label>
                  <input id="reg-phone" name="tel" type="tel" required value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full p-4 rounded-2xl bg-gray-50 border-none outline-none text-xs font-bold" />
                </div>
                <div className="space-y-1">
                  <label htmlFor="reg-gender" className="text-[8px] font-bold text-gray-400 uppercase ml-2">Genere</label>
                  <select id="reg-gender" name="gender" value={gender} onChange={e => setGender(e.target.value as any)} className="w-full p-4 rounded-2xl bg-gray-50 border-none outline-none text-xs font-bold">
                ...
                <div className="space-y-1">
                  ...
                  <input id="reg-dob" name="bday" type="date" required value={dob} onChange={(e) => setDob(e.target.value)} className="w-full p-4 rounded-2xl bg-gray-50 border-none outline-none text-xs font-bold" />
                </div>
              </>
            )}
            ...
            <div className="space-y-1 md:col-span-2">
              ...
              <input id="auth-email" name="email" type="email" autoComplete="username email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full p-4 rounded-2xl bg-gray-50 border-none outline-none text-xs font-bold" />
            </div>
            ...
            <div className="space-y-1">
              ...
              <input 
                ...
                className="w-full p-4 rounded-2xl bg-gray-50 border-none outline-none text-xs font-bold" 
              />
            </div>
            {isRegistering && (
              <div className="space-y-1">
                <label htmlFor="reg-confirm-password" className="text-[8px] font-bold text-gray-400 uppercase ml-2">Conferma Password</label>
                <input id="reg-confirm-password" name="confirm-password" type="password" autoComplete="new-password" required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="w-full p-4 rounded-2xl bg-gray-50 border-none outline-none text-xs font-bold" />
              </div>
            )}
            ...
            {isRegistering && regType === 'collaborator' && (
              <div className="md:col-span-2 grid gap-4 mt-4 p-6 bg-amber-50 rounded-3xl animate-in slide-in-from-top-4">
                <p className="text-[10px] font-bold text-amber-800 uppercase tracking-widest">Dati Staff</p>
                <div className="space-y-1">
                  <label htmlFor="reg-address" className="text-[8px] font-bold text-amber-600 uppercase ml-2">Indirizzo</label>
                  <input id="reg-address" name="address" type="text" required value={address} onChange={(e) => setAddress(e.target.value)} className="w-full p-4 rounded-2xl bg-white border-none outline-none text-xs font-bold" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label htmlFor="reg-avs" className="text-[8px] font-bold text-amber-600 uppercase ml-2">AVS</label>
                    <input id="reg-avs" name="avs" type="text" required value={avs} onChange={(e) => setAvs(e.target.value)} placeholder="756.xxxx..." className="w-full p-4 rounded-2xl bg-white border-none outline-none text-xs font-bold" />
                  </div>
                  <div className="space-y-1">
                    <label htmlFor="reg-iban" className="text-[8px] font-bold text-amber-600 uppercase ml-2">IBAN</label>
                    <input id="reg-iban" name="iban" type="text" required value={iban} onChange={(e) => setIban(e.target.value)} placeholder="CHxx..." className="w-full p-4 rounded-2xl bg-white border-none outline-none text-xs font-bold" />
                  </div>
                </div>
              </div>
            )}
            ...
            {isRegistering && (
               <div className="space-y-1 md:col-span-2">
                <label htmlFor="reg-avatar" className="text-[8px] font-bold text-gray-400 uppercase ml-2">URL Foto Profilo</label>
                <input id="reg-avatar" name="photo" type="text" value={avatar} onChange={(e) => setAvatar(e.target.value)} className="w-full p-4 rounded-2xl bg-gray-50 border-none outline-none text-xs font-bold" />
              </div>
            )}
          </div>
          ...
          <button type="submit" disabled={loading} className="w-full py-5 bg-black text-white font-bold rounded-3xl shadow-2xl hover:bg-amber-700 transition-all uppercase text-[10px] tracking-widest mt-6">
            {loading ? <i className="fas fa-spinner animate-spin"></i> : (isRegistering ? 'Crea Account Kristal' : 'Accedi')}
          </button>
        </form>
        ...
        <div className="mt-10 text-center">
          <button onClick={() => { setIsRegistering(!isRegistering); setErrorMsg(''); }} className="text-[9px] font-bold text-gray-400 hover:text-amber-600 transition-colors uppercase tracking-[0.2em]">
            {isRegistering ? 'Hai già un account? Entra' : 'Nuovo Ospite? Registrati'}
          </button>
        </div>
      </div>
    </div>
